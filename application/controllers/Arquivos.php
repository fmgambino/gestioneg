...
class Arquivos extends MY_Controller
{
    ...
    public function gerenciar()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vArquivo')) {
            $this->session->set_flashdata('error', 'No tiene permiso para ver archivos.');
            redirect(base_url());
        }
    ...
    public function adicionar()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aArquivo')) {
            $this->session->set_flashdata('error', 'No se le permite agregar archivos.');
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->arquivos_model->add('documentos', $data) == true) {
                $this->session->set_flashdata('success', '¡Archivo agregado con éxito!');

                log_info('Añadir un Archivo');

                redirect(site_url('arquivos/adicionar/'));
            } else {
                $this->data['custom_error'] = '<div class="form_error"><p>A ocurrido un erro.</p></div>';
            }
        }
    ...
    public function editar()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'eArquivo')) {
            $this->session->set_flashdata('error', 'No se le permite editar archivos.');
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->arquivos_model->edit('documentos', $data, 'idDocumentos', $this->input->post('idDocumentos')) == true) {
                $this->session->set_flashdata('success', '¡Cambios realizados con éxito!');
                log_info('Cambió un archivo, ID: ' . $this->input->post('idDocumentos'));
    ...
    public function download($id = null)
    {
        ...
        if ($id == null || !is_numeric($id)) {
            $this->session->set_flashdata('error', 'Error! O arquivo não pode ser localizado.');
    ...
    public function excluir()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'dArquivo')) {
            $this->session->set_flashdata('error', 'Usted NO tiene permisos para eliminar el archivo');
            redirect(base_url());
        }

        $id = $this->input->post('id');
        if ($id == null || !is_numeric($id)) {
            $this->session->set_flashdata('error', 'Error! El Archivo no puede ser localizado.');
        ...
        if ($this->db->delete('documentos')) {
            $path = $file->path;
            unlink($path);
            $this->session->set_flashdata('success', 'Archivo Eliminado Éxito');
            log_info('Removeu um arquivo. ID: ' . $id);
        } else {
            $this->session->set_flashdata('error', 'Ocorreu um erro al tratar e eliminar un archivo');
        }
        redirect(site_url('arquivos'));
    }
    ...
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aArquivo')) {
            $this->session->set_flashdata('error', 'No se le permite agregar arquivos.');
            redirect(base_url());
        }
