...
class Sgtos extends MY_Controller
{
    ...
    {
        $this->data['ordens'] = $this->Sgtos_model->getOsAbertas();
        $this->data['ordens1'] = $this->Sgtos_model->getOsAguardandoPecas();
        $this->data['ordens_andamento'] = $this->Sgtos_model->getOsAndamento();
        $this->data['produtos'] = $this->Sgtos_model->getProdutosMinimo();
        $this->data['os'] = $this->Sgtos_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->Sgtos_model->getEstatisticasFinanceiro();
        $this->data['financeiro_mes_dia'] = $this->Sgtos_model->getEstatisticasFinanceiroDia($this->input->get('year'));
        $this->data['financeiro_mes'] = $this->Sgtos_model->getEstatisticasFinanceiroMes($this->input->get('year'));
        $this->data['financeiro_mesinadipl'] = $this->Sgtos_model->getEstatisticasFinanceiroMesInadimplencia($this->input->get('year'));
        $this->data['menupanel'] = 'Panel';
        $this->data['view'] = 'sgtos/panel';
        return $this->layout();
    }

    public function minhaConta()
    {
        $this->data['usuario'] = $this->Sgtos_model->getById($this->session->userdata('id_admin'));
        $this->data['view'] = 'sgtos/minhaConta';
        return $this->layout();
    }

    public function alterarSenha()
    {
        $current_user = $this->Sgtos_model->getById($this->session->userdata('id_admin'));

        if (!$current_user) {
            $this->session->set_flashdata('error', 'Ocorreu um erro ao pesquisar usuário!');
            redirect(site_url('Sgtos/minhaConta'));
        }
        ...
        $result = $this->Sgtos_model->alterarSenha($senha);

        if ($result) {
            $this->session->set_flashdata('success', 'Senha alterada com Éxito');
            redirect(site_url('Sgtos/minhaConta'));
        }
    ...
    public function pesquisar()
    {
        ...
        $data['results'] = $this->Sgtos_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        $this->data['servicos'] = $data['results']['servicos'];
        $this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'sgtos/pesquisa';
        return $this->layout();
    }
    ...
    public function emitente()
    {
        ...
        $this->data['dados'] = $this->Sgtos_model->getEmitente();
        $this->data['view'] = 'sgtos/emitente';
        return $this->layout();
    }
    ...
    public function cadastrarEmitente()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(site_url('Sgtos/emitente'));
        } else {
            ...
            $retorno = $this->Sgtos_model->addEmitente($nome, $cnpj, $ie, $cep, $logradouro, $numero, $bairro, $cidade, $uf, $telefone, $email, $logo);
            if ($retorno) {
    ...
    public function uploadUserImage()
    {
        ...
        $usuario = $this->Sgtos_model->getById($id);
        ...
        $imageUserPath = $image;
        $retorno = $this->Sgtos_model->editImageUser($id, $imageUserPath);
    ...
    public function emails()
    {
        ...
        $this->data['configuration']['base_url'] = site_url('Sgtos/emails/');
    ...
    public function excluirEmail()
    {
        ...
        if ($id == null) {
            ...
            redirect(site_url('Sgtos/emails/'));
        }
    ...
    public function configurar()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($this->Sgtos_model->saveConfiguracao($data) == true) {
                $this->session->set_flashdata('success', 'Configuración del sistema actualizada con éxito');
                redirect(site_url('Sgtos/configurar'));
            } else {
                $this->data['custom_error'] = '<div class="alert">Ocorreu um errro.</div>';
            }
        }

        $this->data['view'] = 'sgtos/configurar';
    ...
    public function atualizarBanco()
    {
        ...
        if ($this->migration->latest() === false) {
            ...
            $this->session->set_flashdata('success', 'Banco de dados atualizado com Éxito');
        }

        return redirect(site_url('Sgtos/configurar'));
    }

    public function atualizarSgtos()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'cSistema')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para configurar o sistema');
            redirect(base_url());
        ...
        if (!$this->github_updater->has_update()) {
            $this->session->set_flashdata('success', 'Seu Sgtos já está atualizado!');
        ...
        if ($success) {
            $this->session->set_flashdata('success', 'Sgtos atualizado com Éxito');
        } else {
            $this->session->set_flashdata('error', 'Erro ao atualizar Sgtos!');
        }
    ...
    public function calendario()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
            $this->session->set_flashdata('error', 'No tienes permiso para visualizar O.S.');
            redirect(base_url());
        }
        ...
        $allOs = $this->Sgtos_model->calendario(
            $start,
            $end,
            $status
        );
        $events = array_map(function ($os) {
            ...
            return [
                ...
                'extendedProps' => [
                    ...
                    'dataInicial' => '<b>Fecha Inicial:</b> ' . $os->dataInicial,
                    'dataFinal' => '<b>Fecha Final:</b> ' . $os->dataFinal,
                    'garantia' => '<b>Garantia:</b> ' . $os->garantia,
                    'status' => '<b>Estado OS:</b> ' . $os->status,
                    'description' => '<b>Descripción/Producto:</b> ' . $os->descricaoProduto,
