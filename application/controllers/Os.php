...
class Os extends MY_Controller
{
    ...
    public function adicionar()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aOs')) {
            $this->session->set_flashdata('error', 'No se le permite agregar O.S.');
            redirect(base_url());
        }
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if (is_numeric($id = $this->os_model->add('os', $data, true))) {
                ...
                $emitente = $this->Sgtos_model->getEmitente()[0];
                ...
                $this->data['custom_error'] = '<div class="alert">Ocurrió un error..</div>';
            }
        }
    ...
    public function enviar_email()
    {
        ...
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'vOs')) {
            $this->session->set_flashdata('error', 'No está permitido enviar SO.S. por e-mail.');
            redirect(base_url());
        }
        ...
        if (!isset($this->data['result']->email)) {
            $this->session->set_flashdata('error', 'El cliente no tiene correo electrónico registrado.');
            redirect(site_url('os'));
        }
        ...
        if (!isset($this->data['emitente'][0]->email)) {
            $this->session->set_flashdata('error', 'Registrar los datos del emisor');
            redirect(site_url('os'));
        }
        ...
        if ($this->data['configuration']['os_notification'] != 'nenhum') {
            ...
            if ($ValidarEmail) {
                if (empty($this->data['result']->email) || !filter_var($this->data['result']->email, FILTER_VALIDATE_EMAIL)) {
                    $this->session->set_flashdata('error', 'Por favor complete el correo electrónico del cliente');
            ...
            if ($enviouEmail) {
                $this->session->set_flashdata('success', 'El correo electrónico se está procesando y se enviará en breve.');
                log_info('Enviou e-mail para o cliente: ' . $this->data['result']->nomeCliente . '. E-mail: ' . $this->data['result']->email);
                redirect(site_url('os'));
            } else {
                $this->session->set_flashdata('error', 'Ocurrió un error al enviar el correo electrónico');
                redirect(site_url('os'));
            }
        }

        $this->session->set_flashdata('success', 'El sistema tiene una configuración activada para no notificar. Póngase en contacto con el administrador.');
        redirect(site_url('os'));
    }
        ...
        if ($produtos = $this->os_model->getProdutos($id)) {
            ...
            if ($this->data['configuration']['control_estoque']) {
                foreach ($produtos as $p) {
                    ...
                    log_info('Stock: Produto id ' . $p->produtos_id . ' devuelto a stock. Cantidad: ' . $p->quantidade . '. Motivo: Cancelamento/Exclusão');
                }
            }
        }
    }
    ...
    public function excluir()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'dOs')) {
            $this->session->set_flashdata('error', 'No se le permite eliminar O.S.');
            redirect(base_url());
        }
        ...
        if ($os == null) {
            ...
            if ($os == null) {
                $this->session->set_flashdata('error', 'Error al intentar eliminar la O.S.');
        ...
        if ($os->idCobranca != null) {
            if ($os->status == "canceled") {
                ...
                $this->session->set_flashdata('error', 'Hay un cargo asociado con esta O.S., ¡primero debe cancelar y/o eliminar el cargo!');
        ...
        $this->session->set_flashdata('success', 'OS eliminada con Éxito');
        redirect(site_url('os/gerenciar/'));
    }
    ...
    public function adicionarProduto()
    {
        ...
        $os = $this->os_model->getById($id);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Error al intentar insertar producto en la OS.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
    ...
    public function excluirProduto()
    {
        ...
        $os = $this->os_model->getById($idOs);
        if ($os == null) {
            $this->session->set_flashdata('error', 'Error al intentar eliminar producto en la OS.');
            redirect(base_url() . 'index.php/os/gerenciar/');
        }
    ...
    public function adicionarDesconto()
    {
        if ($this->input->post('desconto') == "") {
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(400)
                ->set_output(json_encode(['messages' => 'Compo descuento esta vacío.']));
        } else {
            $idOs = $this->input->post('idOs');
            $data = [
            ...
            if ($this->os_model->edit('os', $data, 'idOs', $idOs) == true) {
                ...
                return $this->output
                    ...
                    ->set_output(json_encode(['result' => true, 'messages' => 'Descuento aplicado con Éxito']));
            } else {
    ...
    public function faturar()
    {
        ...
        if ($this->form_validation->run('receita') == false) {
            ...
            if ($this->os_model->add('lancamentos', $data) == true) {
                ...
                $this->session->set_flashdata('success', 'OS faturada com Éxito');
                $json = ['result' => true];
    ...
    private function enviarOsPorEmail($idOs, $remetentes, $assunto)
    {
        ...
        $dados['emitente'] = $this->Sgtos_model->getEmitente();
