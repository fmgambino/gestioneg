...
<div class="new122">
    ...
    <a href="<?php echo base_url(); ?>index.php/garantias/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px">
      <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2">Plazo de garantía</span></a>
<?php } ?>
        ...
        <table id="tabela" class="table table-bordered ">
            ...
            <tbody>
                <?php
                    if (!$results) {
                        echo '<tr>
                                <td colspan="6">No se registraron Terminos y Cond. de Garantías</td>
                                </tr>';
                    }
                    foreach ($results as $r) {
                        ...
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vGarantia')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/garantias/visualizar/' . $r->idGarantias . '" class="btn-nwe" title="Ver más detalles"><i class="bx bx-show bx-xs"></i></a>';
                        ...
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dGarantia')) {
                            echo '<a href="#modal-excluir" role="button" data-toggle="modal" garantia="' . $r->idGarantias . '" class="btn-nwe4" title="Eliminar"><i class="bx bx-trash-alt bx-xs"></a>';
                        }
                        echo '</td>';
                        echo '</tr>';
                    } ?>
...
<div id="modal-excluir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>index.php/garantias/excluir" method="post">
        <div class="modal-header">
            ...
            <h5 id="myModalLabel">Eliminar plazo de garantía</h5>
        </div>
        <div class="modal-body">
            <input type="hidden" id="idGarantias" name="idGarantias" value="" />
            <h5 style="text-align: center">Desea realmente eliminar este plazo de garantía?</h5>
        </div>
