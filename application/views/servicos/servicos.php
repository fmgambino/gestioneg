...
<div class="new122">
    ...
    <a href="<?php echo base_url() ?>index.php/servicos/adicionar" class="button btn btn-mini btn-success" style="max-width: 160px">
      <span class="button__icon"><i class='bx bx-plus-circle'></i></span><span class="button__text2"> Servicio</span></a>
<?php } ?>
        ...
        <table id="tabela" class="table table-bordered ">
            ...
            <tbody>
                <?php
                    if (!$results) {
                        echo '<tr>
                                <td colspan="5">Sin servicio registrado</td>
                              </tr>';
                    }
                    foreach ($results as $r) {
                        ...
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eServico')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/servicos/editar/' . $r->idServicos . '" class="btn-nwe3" title="Editar Servicio"><i class="bx bx-edit bx-xs"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dServico')) {
                            echo '<a href="#modal-excluir" role="button" data-toggle="modal" servico="' . $r->idServicos . '" class="btn-nwe4" title="Eliminar Servicio"><i class="bx bx-trash-alt bx-xs"></i></a>  ';
                        }
                        echo '</td>';
                        echo '</tr>';
                    } ?>
